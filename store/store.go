...
//
// 默认实现是 JSONStore(单目录 + 几个 JSON 文件),满足 SDK 嵌入场景。
// 未来可平替为 Redis / SQLite 而无需改动上层。
package store

import "github.com/doptime/dopharness/chunk"

// FileMeta 记录一个物理文件的索引状态,用于增量判定与删除检测。
type FileMeta struct {
	Path     string   `json:"path"`      // 相对项目根
	ModTime  int64    `json:"mtime"`     // 文件最后修改时间(秒)
	Hash     string   `json:"hash"`      // 整文件内容 hash
	ChunkIDs []string `json:"chunk_ids"` // 该文件贡献的所有 chunk ID
}
...
type ChunkStore interface {
	...
	// ChunksByName 按 Name 返回所有匹配的 chunk。用于 LLM 写错 ID 时的模糊回退。
	ChunksByName(name string) []*chunk.Chunk

	// ChunksByFile 返回指定文件下的所有 chunk。
	...
	//
	// 返回"最终入库"的 chunk 列表(已填好 ID、UpdatedAt、ContentHash)。
